var Hm = (function() {
    ...
    Hm.prototype.takeCuantasFromLibres = function(rows, cuantas){
      ...
      for (var i = 0; i < rows.length; ++i) {
        ...
        var arr = [rows[i].hab_Id, rows[i].fechaEntrada,rows[i].fechaSalida,rows[i].cliente,rows[i].tel,rows[i].correo,rows[i].adultos,rows[i].ninos];
        arrs.push(arr);
      };
      return arrs;
    };
